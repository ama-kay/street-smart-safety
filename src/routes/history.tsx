...
const events = [
  { date: "Oct 12, 2025", time: "9:42 PM", status: "Delivered", location: "Downtown, Main St", ok: true },
  { date: "Sep 28, 2025", time: "11:15 PM", status: "Delivered", location: "Park Avenue", ok: true },
    ...
    <MobileShell>
      ...
      <div className="flex-1 px-6 pt-6 pb-10 overflow-y-auto">
        ...
        <div className="space-y-3">
          {events.map((e, i) => (
            <div key={i} className="bg-card border border-border rounded-2xl p-4 flex items-start gap-4 shadow-card">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
                e.ok ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"
              }`}>
                {e.ok ? <Check className="w-5 h-5" strokeWidth={3} /> : <AlertCircle className="w-5 h-5" />}
              </div>
